...
u_oodl(char *pre, char *pos)
{
    ...
    if (expre_pos.topo >= 0)
    {
        ...
        {
            // primeiro procura operadores do tipo += *=

            if ( eh_op_assign(expre_pos.no[i].tipo) )   // procura operadores nao atribuicao
            {
                expre_pos.no[i].tipo = OP_ASS_PLAIN;
                strcpy(&(expre_pos.buf[expre_pos.no[i].nome]), "=");
 //             expre_completa(&expre_pos); // expre_valida chama expre_completa
            ...
            if ( ! eh_operador(expre_pos.no[i].tipo) )   // procura operadores nao atribuicao
                continue;
            expre_completa(&expre_pos);

            if ( eh_operador_unario(expre_pos.no[i].tipo))
            {
                // op unario apenas remove o operador
 ...
 //             expre_completa(&expre_pos); // expre_valida chama expre_completa
            ...
            {
                // operador binario remove operador e uma subexpressao de cada vez
                expre_pos.no[i].tipo = NOP;

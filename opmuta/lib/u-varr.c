...
char	*var_name;
   ...
   if (expre_pos.topo >= 0)
   {
	   ...
	   if ( ! expre_e_referencia(&expre_pos, i)  ||
		! expre_e_array(&expre_pos, i, tipo, ptar))
			     continue;
	   ...
	   {
		if ( (Operador == U_VGAR && e_local(q->nome)) ||
		     ! e_array(q->tipo, q->ptar) ||

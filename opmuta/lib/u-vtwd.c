...
{
  Operador = U_VTWD;
  set_new_inic(&x, "PRED\0SUCC\0")
  u_vdtr_twd(pre, pos);
}

u_vdtr_twd(char *pre, char *pos)
{
char	*p, *set_prim(), *set_succ(), tipo[100], ptar[100]; 
int	i, j, fim;
int	u;
OSET    r, x
  ...
  if (max ==0) max--;
   if (expre_pos.topo >= 0)
	...
	ex_aux1 = expre_pos;
	for (i = 0; i <= expre_pos.topo; i++, expre_pos = ex_aux1)
	   {		/* procura referencia do tipo desejado */
		...
		/*for (;p != NULL;p = set_succ(p, set_por))*/

...
char    *c;
  ...
  if (expre_pos.topo >= 0)
	...
	{		/* procura referencia do tipo desejado */
	   ...
	   compute_setField(bufmen, tipo, ptar, &r);
           var_set_dup(&r, p);
           nseq = var_set_card(p);		


	   for (n = max, q = var_get_rand(p, Operador); q != NULL && n != 0;	
                             q = var_get_rand(p, Operador),n--)
	   {	/* troca pelas outras variaveis */ 

		if (strcmp(NOME((&expre_pos), i), q->nome) == 0 ||

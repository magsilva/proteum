...
void DirVarRepPar(char *pre, char *pos)
{
...
char	*var_name;
   ...
   if (expre_pos.topo >= 0)
   {
		     ...
		     n != 0 && q != NULL; q = var_get_rand(p, Operador), n--)
	   {
		...
		{
		   seq++;
		   ins_expre_apos(&expre_pos, "*", OP_1_DEREF, i);
		   expre_completa(&expre_pos);
	   	   expre_exec_tipo(&expre_pos,
                              expre_pos.no[i].final, i, tipo, ptar);
		}
...
void IndVarRepPar(char *pre, char *pos, int ret)
{
SIMBOLO *q, *var_get_rand();
int     i, j, n, max, seq, nseq;
int     u, k, t;

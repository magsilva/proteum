...
tty_save(int fd)
{
   ...
   {
	msg("internal error: tcgetattr");
	return ERRO;
   }

   if (ioctl(fd, TIOCGWINSZ, (char *) &wsize) < 0)
   {
	msg("internal error: TIOCGWINSZ");
	return ERRO;
   }
   return OK;
}
...
tty_copy(int fd)
{
   ...
   {
	msg("tcsetattr error on slave pty");
	return ERRO;
   }
   if (ioctl(fd, TIOCSWINSZ, &wsize) < 0)
   {
	msg("TIOCSWINSZ error on slave pty");
	return ERRO;
   }
}  
